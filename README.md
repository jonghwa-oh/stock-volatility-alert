...
- 🔔 **알림 제어**: `/alarm_off`로 밤에 알림 끄기, `/alarm_on`으로 다시 켜기
- 🤖 **텔레그램 봇 커맨드**: `/add`, `/remove`, `/status`, `/morning` 등 편리한 관리
...
🔔 알림 제어:
/alarm_on - 알림 켜기
/alarm_off - 알림 끄기 (잠잘 때 유용!)
/alarm_status - 현재 알림 상태 확인
```
...
→ 🔔 알림 상태: 활성화
   📊 관심 종목: 4개
   💰 투자금액: 1,000,000원
   끄려면 /alarm_off 를 입력하세요.
```

**💡 TIP:** 밤 12시~아침 7시 반까지 알림을 끄고 싶을 때 `/alarm_off`를 사용하세요!
